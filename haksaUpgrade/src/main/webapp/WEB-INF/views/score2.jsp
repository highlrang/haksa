	...
	<form method="get">
		...
		<c:choose>
			<c:when test="${empty complete_list }">
				<span style="margin: 5%;"><input type="submit" formaction="readyScoreInsert" value="성적입력"/></span><br>
			</c:when>
					
			<c:otherwise>
				<c:if test="${complete_list.contains(score.getUser_id()) }">
					<span style="margin: 5%"><input type="submit" formaction="readyScoreUpdate" value="성적 수정"/></span><br>				
				</c:if>
				<c:if test="${none_list.contains(score.getUser_id()) }">
					<span style="margin: 5%;"><input type="submit" formaction="readyScoreInsert" value="성적 입력"/></span><br>
				</c:if>
			</c:otherwise>
