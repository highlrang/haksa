		...
		<td height="1000px" style="float: right; margin-right: 5%;">
			...
			<form action="logout" method="post" style="height: 100%">
				...
				<div id="user_info" style="display:block; background-color: beige; padding: 3%; text-align: left;">
					<div>이름 : ${user.getUser_name() }</div>
					<div>전공 : ${user.getUser_major() }</div>
					
					<c:if test="${not empty user.getUser_minor() }">
						<div>복수전공 : ${user.getUser_minor() }</div>
					</c:if>

					<div>전화번호 : ${user.getUser_phone() }</div>
					<div>이메일 : ${user.getUser_email() }</div>
				</div>
				<br>
